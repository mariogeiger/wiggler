...
public struct KLTTracker {
    ...
    private func trackDirection(prev: Pyramid, cur: Pyramid, x: Float, y: Float) -> KLTResult {
        ...
        // Columns are x/y translation and the four affine displacements at the window radius.
        var jacobian = [Double](repeating: 0, count: n * 6)
        for level in stride(from: levelCount - 1, through: 0, by: -1) {
            let p = prev.levels[level], c = cur.levels[level]
            let margin = Double(half + 2)
            if px < margin || py < margin || px > Double(p.width - 1) - margin || py > Double(p.height - 1) - margin {
            ...
            guard eig / Double(n) >= Double(minEigenvalue), det >= 1e-12 else { return failure }
            let pivotFloor = (0..<6).map { h[$0 * 6 + $0] }.max()! * 1e-10
            ...
            for row in 0..<6 {
                for col in 0...row {
                    ...
                    if row == col {
                        guard value.isFinite, value > pivotFloor else { return failure }
            ...
            for _ in 0..<maxIterations {
                ...
                for dy in -half...half {
                    for dx in -half...half {
                        ...
                        for row in 0..<6 { b[row] += jacobian[i * 6 + row] * d }
                        i += 1
                    }
                }
                for row in 0..<6 {
                ...
                guard determinant.isFinite, abs(determinant) > 1e-9 else { return failure }
                ...
                gx -= ux
                gy -= uy
                let shapeStep = max(abs(n00 - a00) + abs(n01 - a01), abs(n10 - a10) + abs(n11 - a11)) * Double(half)
                a00 = n00
                a01 = n01
                a10 = n10
                a11 = n11
